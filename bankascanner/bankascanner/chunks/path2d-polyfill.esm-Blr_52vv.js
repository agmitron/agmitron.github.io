...
function j(n) {
  ...
  return r[0] !== "M" && r[0] !== "m" || r.replace(Z, function(t, o, l) {
    ...
    if (u === "m" && s.length > 2 && (a.push(F([h], s.splice(0, 2))), u = "l", h = h === "m" ? "l" : "L"), s.length < q[u])
      return "";
    for (a.push(F([h], s.splice(0, q[u]))); s.length >= q[u] && s.length && q[u]; )
      a.push(F([h], s.splice(0, q[u])));
    return "";
  }), a;
}
function B(n, a) {
    ...
    switch (b) {
      ...
      case "A":
        ...
        b === "a" ? (r += e[6], t += e[7]) : (r = e[6], t = e[7]), p = e[1], f = e[2], P = e[3] * Math.PI / 180, s = !!e[4], u = !!e[5], h = { x: r, y: t }, v = {
          x: (m.x - h.x) / 2,
          y: (m.y - h.y) / 2
        }, B(v, -P), C = v.x * v.x / (p * p) + v.y * v.y / (f * f), C > 1 && (C = Math.sqrt(C), p *= C, f *= C), k = {
          x: p * v.y / f,
          y: -(f * v.x) / p
        }, I = p * p * f * f, A = p * p * v.y * v.y + f * f * v.x * v.x, u !== s ? H(k, Math.sqrt((I - A) / A) || 0) : H(k, -Math.sqrt((I - A) / A) || 0), l = Math.atan2((v.y - k.y) / f, (v.x - k.x) / p), o = Math.atan2(-(v.y + k.y) / f, -(v.x + k.x) / p), B(k, P), J(k, (h.x + m.x) / 2, (h.y + m.y) / 2), n.save(), n.translate(k.x, k.y), n.rotate(P), n.scale(p, f), n.arc(0, 0, 1, l, o, !u), n.restore();
        break;
      case "C":
...
K(window);
O(window);
//# sourceMappingURL=path2d-polyfill.esm-Blr_52vv.js.map
